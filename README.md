...
Pass `--json` to print the same report as JSON instead of a table, for scripts and dashboards
(Node implementation only):

```
$ svc status --json
{"checkedAt":"2026-09-24T09:15:00.000Z","components":[{"name":"api","ok":true,"latencyMs":12,"detail":"200 OK"},{"name":"database","ok":true,"latencyMs":3,"detail":"connections 4/50"},{"name":"queue","ok":true,"latencyMs":8,"detail":"depth 0"}]}
```
